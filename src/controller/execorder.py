...
async def inner_order_exec(ws:WebSocketClientProtocol,order_request,restartConfirmEvent:asyncio.Event,oouter_order_dict):
...
async def order_exec(ws:WebSocketClientProtocol,order_request,restartConfirmEvent:asyncio.Event,outer_order_dict,host,port,sftp_user,sftp_password):
    ...
    myLogger.info(f"receive instruction from Server:{order_request["Headers"]["order_type"]}")
    if order_request["Headers"]["isInnerOrder"] == True:
