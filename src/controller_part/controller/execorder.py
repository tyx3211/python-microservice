...
async def inner_order_exec(
    ...
    if order_request["Headers"]["order_type"] == "restart":
        ...
        await ws.send(
            json.dumps(
                {
                    "type": "upload_instruction_result",
                    "status": "success",
                    "Headers": {"message": "wait Server Confirm"},
                    "data": {},
                }
            )
        )
        try:
            await asyncio.wait_for(
                restartConfirmEvent["event"].wait(), timeout=20
            )  # 对于重启指令，额外有一个确认步骤，但是最多等3s
        ...
        try:
            ...
            if (
                await sftp_upload_log(
                    device_id,
                    config.device_log_local_path,
                    remote_dir,
                    config.sftp_host,
                    config.sftp_port,
                    ...
                    json.dumps(
                        {
                            "type": "upload_instruction_result",
                            "status": "fail",
                            "Headers": {"message": "fail to upload log."},
                            "data": {},
                        }
                    )
                )
                    ...
                    json.dumps(
                        {
                            "type": "upload_instruction_result",
                            "status": "success",
                            "Headers": {"message": "successfully upload log."},
                            "data": {},
                        }
                    )
                )
...
async def outer_order_exec(ws: WebSocketClientProtocol, order_request):
            ...
            json.dumps(
                {
                    "type": "upload_instruction_result",
                    "status": "fail",
                    "Headers": {"message": "order not in config"},
                    "data": {},
                }
            )
        )
    try:
        ...
        if "params" not in order_request["data"] or (
            not isinstance(order_request["data"]["params"], dict)
        ):
            execFunc()
            ...
            json.dumps(
                {
                    "type": "upload_instruction_result",
                    "status": "success",
                    "Headers": {"message": "successfully exec order."},
                    "data": {},
                }
            )
        )
            ...
            json.dumps(
                {
                    "type": "upload_instruction_result",
                    "status": "fail",
                    "Headers": {"message": f"Error in exec order: {e}"},
                    "data": {},
                }
            )
        )
...
async def order_exec(
    ws: WebSocketClientProtocol, order_request, restartConfirmEvent: asyncio.Event
):
    myLogger.info(
        "receive instruction from Server:" + str(order_request["Headers"]["order_type"])
    )
