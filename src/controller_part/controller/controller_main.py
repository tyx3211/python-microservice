...
latestBussinessTimeStamp = (
    None  # 除心跳外上次其它业务成功执行的时间戳，用于规范心跳延时时间
)
    ...
    try:
        ...
        if config.device_id == None:
            await ws.send(
                json.dumps(
                    {
                        "device_sn": config.device_hardware_sn,
                        "device_model": config.device_hardware_model,
            ...
            await ws.send(
                json.dumps(
                    {"device_id": config.device_id, "password": config.device_password}
                )
            )
    ...
    while True:
        try:
            ...
            if response_data.get("type") == "ping_pong":  # 接收到pong帧
                ...
                response_data.get("type") == "send_statusInfo_response"
            ):  # 接收到对状态上传的回应
                ...
                # print("hello")                 # 接收到下达指令
                asyncio.create_task(
                    execOrder(ws, response_data, Events["restart_confirm"])
                )  # 特别注意！！！ ： 由于在execOrder模块下，我们可能会有await确认的操作（如Restart，确认需要服务端再发信），因此一定不能await调用构成依赖！否则会导致Restart依赖服务端确认才能执行完（否则超时），但是接收服务端确认的public_recv又依赖execOrder结果，导致Restart只能超时，因此这里应该异步启动新任务
                ...
                Events["restart_confirm"][
                    "data"
                ] = response_data  # 注意由于python传递了Events的引用，因此后序set的Events["restart_confirm"]["event"]照样可以传递到相应execOrder处理模块下
...
async def ping_pong(ws: WebSocketClientProtocol, event: asyncio.Event):
    ...
    while True:
        try:
            ...
            await ws.send(
                json.dumps({"type": "ping_pong", "Headers": {}, "data": "ping"})
            )
            last_send_ping = time.time()
            myLogger.info(f"send ping")

            await asyncio.wait_for(
                event["event"].wait(), timeout=3
            )  # 发出ping到接收pong最多等3秒
            event["event"].clear()
            ...
            await asyncio.sleep(
                wait_time_from(last_send_ping, 5)
            )  # 等到了pong，就有5s的容许期
        except asyncio.TimeoutError:
...
async def uploadStatusInfo(ws: WebSocketClientProtocol, event: asyncio.Event):
    ...
    while True:
        try:
            ...
            await ws.send(
                json.dumps(
                    {"type": "upload_status_info", "Headers": {}, "data": StatusInfo}
                )
            )
            ...
            await asyncio.sleep(
                wait_time_from(last_send_status, 15)
            )  # 每隔15s发送状态报文
        except asyncio.TimeoutError:
            if (
                ws != Ws
            ):  # 若连接断开，则不再发送状态报文(只需对非心跳活动检测，因为是由ping_pong决定断开的，这一步是为了在断开后正确析构未释放的资源)
                break
...
async def execOrder(
    ws: WebSocketClientProtocol, order_request, restartConfirmEvent: asyncio.Event
):
    try:
    ...
    while retries < config.max_retries:
        try:
            ...
            async with websockets.connect(
                config.server_url
            ) as ws:  # 发起webSocket连接，连接成功后webSocket对象引用给到ws
                ...
                events = get_events()
                asyncio.create_task(
                    public_recv(ws, events)
                )  # 异步添加公共接受点任务（在其中也解决了对指令执行的调动）
                asyncio.create_task(
                    uploadStatusInfo(ws, events["receive_statusSend_response"])
                )  # 异步添加上传状态信息任务
                await ping_pong(
                    ws, events["ping_pong"]
                )  # 开始ping-pong 心跳，心跳结束即进入重连
        except Exception as e:
...
def startControllerBasicApp(
    ...
    config.Set(
        ...
        device_hardware_sn=device_hardware_sn,
        device_hardware_model=device_hardware_model,
        device_log_dir=device_log_dir,
        device_log_name=device_log_name,
        max_retries=max_retries,
        outer_order_dict=outer_order_dict,
    )
    SetLogMessage(
        myLogger, config.device_log_dir, config.device_log_local_path
    )  # 配置完config后再配置日志
