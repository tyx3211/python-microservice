        ...
        result["result"] = response.json(
            {
                "status": "fail",
                "type": "GENERAL_ERROR",
                "message": "please provide legal json!",
            },
            status=400,
        )
        return False
...
def dealException(e: Exception):
    if isinstance(e, DeviceOPError):
        return response.json(
            {"status": "fail", "type": "DEVICEOP_ERROR", "message": str(e)},
            status=e.Error_Code,
        )
    elif isinstance(e, GroupOPError):
        return response.json(
            {"status": "fail", "type": "GROUPOP_ERROR", "message": str(e)},
            status=e.Error_Code,
        )
    elif isinstance(e, RelationOPError):
        return response.json(
            {"status": "fail", "type": "RELATIONOP_ERROR", "message": str(e)},
            status=e.Error_Code,
        )
        ...
        return response.json(
            {
                "status": "fail",
                "type": "DATABASE_ERROR",
                "message": "Maybe JSON parameter type incorrect(please check) or DataBase Inner Error",
            },
            status=500,
        )
    else:
        return response.json(
            {"status": "fail", "type": "INNER_ERROR", "message": "Server Inner Error"},
            status=500,
        )
...
async def setup_db(app, loop):
    ...
    if not os.path.exists(log_dir):
        ...
        os.chmod(
            "/var/log/devices_management", stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO
        )  # 777权限，所有人都可读写执行(保证边端设备日志上传)


@app.listener("after_server_stop")
async def close_db(app, loop):
    # if len(DevicesOnlineState)!= 0:
    await deviceOP.update_state_to_offline(
        device_ids=tuple(DevicesOnlineState.keys())
    )  # 所有设备状态改为offline
...
async def device_add(request: Request):
        ...
        return response.json(
            {
                "status": "fail",
                "type": "GENERAL_ERROR",
                "message": "password shoule be string!",
            },
            status=400,
        )
    ...
    try:
        ...
        hashed_password = bcrypt.hashpw(
            dic["password"].encode("utf-8"), bcrypt.gensalt()
        )  # 将用户密码加盐哈希
...
@app.get(
    "/service/devices_manage/device_basic_SnModel_query/<device_sn>/<device_model>"
)
...
async def group_add(request):
    ...
    try:
        ...
        return response.json(
            {"status": "success", "data": {"group_id": group_id}}, status=200
        )
    except Exception as e:
        return dealException(e)
...
async def relation_add(request):
    result = {}  # 传出参数
    if (
        checkJsonParams(request, relation_required_fields, result) is False
    ):  # 检查出错误
        return result["result"]
...
async def relation_del(request):
    result = {}
    if (
        checkJsonParams(request, ["device_id", "group_id"], result) is False
    ):  # 检查出错误
        return result["result"]
...
async def relation_group_modify(request):
    ...
    result = {}  # 传出参数
    if (
        checkJsonParams(request, ["device_id", "group_id", "new_group_id"], result)
        is False
    ):  # 检查出错误
        return result["result"]
...
async def relation_device_modify(request):
    ...
    result = {}  # 传出参数
    if (
        checkJsonParams(request, ["device_id", "group_id", "new_device_id"], result)
        is False
    ):  # 检查出错误
        return result["result"]
...
async def delete_device_all_relation(request):
    result = {}
    if (
        checkJsonParams(request, ["device_id"], result) is False
    ):  # 至少需要提供device_id
        return result["result"]
...
async def device_statusInfo_query(request, device_id):
    ...
    if result is None:
        return response.json(
            {"status": "fail", "message": "wrong device_id OR offline device."}
        )
    else:
        # print(result)
...
async def give_order(request):
    result = {}
    if (
        checkJsonParams(request, ["device_id", "order"], result) is False
    ):  # 至少需要提供device_id和order
        return result["result"]
    ...
    try:
        ...
        deal_dic = {
            "type": "give_order",
            "Headers": {"order_type": dic["order"]},
            "data": {"device_id": dic["device_id"]},
        }
        # print(deal_dic)
