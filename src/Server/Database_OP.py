...
def WrapperToJson(
    OriginDict: dict, isDevice=False, isGroup=False, isRelation=False
) -> dict:
    if isDevice:
        result = {}
        ...
        result["software"]["last_update"] = OriginDict["software_last_update"].strftime(
            "%Y-%m-%d"
        )  # 额外转化Date对象
        result["nic"] = [{}, {}]
...
# 封装update_sql生成函数
def getUpdateSQL(
    sqlPartBefore: str, update_info, referece_list: list, sqlPartAfter: str
) -> str:
    Part_str = ""
    index = 0
...
# 再封装一些常用子操作(多为查询)
async def isDeviceExist(
    cursor, device_id=None, SN_Model=None
):  # SN_Model是(hardware_sn,hardware_model)元组
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    device_info = addTimestamp(
                        device_info, isCreated=True
                    )  # 包装created_time和updated_time
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    timestamp = math.floor(time.time() * 1000)
                    sql = (
                        f"UPDATE devices SET dev_state='offline',updated_time={timestamp} WHERE device_id IN "
                        + "("
                        + ("%s," * (len(device_ids) - 1))
                        + "%s);"
                    )
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    group_info = addTimestamp(
                        group_info, isCreated=True
                    )  # 包装created_time和updated_time
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    # 再查询分组下是否有设备
                    sqlTest = (
                        """SELECT device_id FROM relations WHERE group_id=%s LIMIT 1;"""
                    )
                    ...
                    sql = getUpdateSQL(
                        ...
                        devices_update_info,
                        device_updated_fields,
                        "WHERE device_id IN (SELECT device_id FROM relations WHERE group_id=%s);",
                    )
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    # 再查询设备是否在分组下
                    sqlTest = (
                        """SELECT group_id FROM relations WHERE device_id=%s LIMIT 1;"""
                    )
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    # 先删每个设备的设备-分组关系
                    sql = (
                        """DELETE FROM relations WHERE device_id IN ("""
                        + ("%s," * (len(TestResult) - 1))
                        + "%s);"
                    )
