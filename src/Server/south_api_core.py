...
device_websocket_map = (
    {}
)  # 用于保存设备和其对应的webSocket连接的字典,并顺带保存和该device和当前webSocket关联的giveOrderEvent
...
async def loginCheck(ws: WebSocketClientProtocol, result):
    try:
        ...
        if "password" not in loginInfo:
            await ws.send(
                json.dumps({"status": "fail", "message": "Not provide password"})
            )
            return False
        if "device_id" in loginInfo:
            ...
            if (
                bcrypt.checkpw(
                    loginInfo["password"].encode("utf-8"),
                    queryResult["password"].encode("utf-8"),
                )
                is False
            ):
                await ws.send(
                    json.dumps({"status": "fail", "message": "password Not Correct"})
                )
                return False
            else:
                await ws.send(
                    json.dumps(
                        {
                            "status": "success",
                            "data": {"device_id": queryResult["device_id"]},
                        }
                    )
                )
            ...
            try:
                queryResult = await deviceOP.query(
                    SN_Model=(loginInfo["device_sn"], loginInfo["device_model"])
                )
                # print(queryResult)
            except Exception as e:
                await ws.send(json.dumps({"status": "fail", "message": str(e)}))
...
DeviceFailureCount = (
    {}
)  # 心跳失败计数统计字典，初值为0，边端状态上传未接收到或者边端指令回应未接收到亦可贡献，至三可判断边端设备断线
...
async def TransDeviceState(
    device_id, ToState
):  # 由于在线状态其实是即时的，不需要持久化保存，其实可以去除数据库表中的dev_state以提高效率（以后的优化考虑）
    ...
    while True:
        try:
            ...
            if response_data.get("type") == "ping_pong":
                await ws.send(
                    json.dumps({"type": "ping_pong", "Headers": {}, "data": "pong"})
                )  # 处理心跳
                ...
                await ws.send(
                    json.dumps(
                        {"type": "send_statusInfo_response", "Headers": {}, "data": {}}
                    )
                )  # 处理状态报文回信
...
async def giveOrder(device_id, order):
    if (device_id not in DevicesOnlineState) or (
        DevicesOnlineState[device_id] == "offline"
    ):
    ...
    if (device_id in devices_instruction_dealing) and devices_instruction_dealing.get(
        device_id
    ) == True:  # 对单设备的并发控制
    ...
    try:
        ...
        await asyncio.wait_for(
            ReceiveResultEvent["event"].wait(), timeout=10
        )  # 当边端回应时可继续，但不能超过10s，否则认为这次指令下达失败
        ReceiveResultEvent["event"].clear()

        if order["Headers"]["order_type"] == "restart":  # 对于重启，额外确认
            try:
                await Ws.send(
                    json.dumps({"type": "restart_confirm", "Headers": {}, "data": {}})
                )
            except Exception as e:
                pass
        ...
        if result["status"] == "success":
            ...
            return response.json(
                {"status": "fail", "message": result["Headers"]["message"]}
            )
    except Exception:
        devices_instruction_dealing[device_id] = False
...
async def Ws_Serve_Core(ws: WebSocketClientProtocol):
    ...
    await TransDeviceState(device_id, "online")
    device_websocket_map[device_id] = {
        "ws": ws,
        "receiveOrderResultEvent": Events["receive_instruction_result"],
    }
